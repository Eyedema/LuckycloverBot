...
def get_schedule(dayN, nthDay):
    this_month_schedule = calendar.Calendar(dayN).monthdatescalendar(
        datetime.today().year, datetime.today().month)[nthDay][0]
    next_month_schedule = calendar.Calendar(dayN).monthdatescalendar(
        datetime.today().year, datetime.today().month + 1)[nthDay][0]
    return str(this_month_schedule) if this_month_schedule > datetime.today(
    ).date() else str(next_month_schedule)
...
def write_log(update):
    ...
    with open('/home/Eyedema/luckycloverbot/obj/botlog.log', 'a+') as file:
        file.write(':: {:%Y-%b-%d %H:%M:%S} {} @{} (ID:{}) searched for {}\n'.format(
            datetime.now(), name, username, str(chat_id), text))
